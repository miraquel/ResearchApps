/**
 * ItemDept Index Page Component
 * Handles listing, filtering, sorting, and pagination of item departments with HTMX
 */
function itemdeptIndex() {
    return {
        isLoading: false,
        isExporting: false,
        sortBy: 'ItemDeptId',
        ...
        init() {
            ...
            document.body.addEventListener('htmx:beforeRequest', (e) => {
                if (e.detail.target && e.detail.target.id === 'itemdept-list-container') {
                    this.isLoading = true;
                }
            });
        ...
        clearFilters() {
            ...
            this.sortBy = 'ItemDeptName';
            this.sortAsc = true;
            this.fetchList();
        },
        
        /**
         * Fetch item department list with current filters, sorting, and pagination
         * Uses HTMX to load partial view into container
         * @param {number} page - Page number to fetch (default: 1)
         * @returns {void}
         */
        fetchList(page = 1) {
            const container = document.getElementById('itemdept-list-container');
            if (!container) return;
            ...
            const url = `/ItemDepts/List?${params.toString()}`;
            
            // Use htmx to fetch
            htmx.ajax('GET', url, {
                target: '#itemdept-list-container',
                swap: 'innerHTML',
        ...
        /**
         * Export item departments to Excel with current filters and sorting
        ...
        async exportToExcel() {
            ...
            try {
                ...
                const response = await fetch(`/api/ItemDepts/export?${params.toString()}`, {
                    method: 'GET',
                    headers: {
                ...
                a.href = url;
                a.download = `ItemDepts_${new Date().toISOString().slice(0, 10)}.xlsx`;
                ...
                console.error('[ItemDept Index] Export error:', error);
...
window.itemdeptIndex = itemdeptIndex;
