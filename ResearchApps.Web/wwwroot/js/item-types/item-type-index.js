/**
 * Item Type Index Page Component
 * Handles listing, filtering, sorting, and pagination of item types with HTMX
 */
    ...
    return {
        ...
        init() {
            ...
            document.body.addEventListener('htmx:beforeRequest', (e) => {
                if (e.detail.target && e.detail.target.id === 'item-type-list-container') {
                    this.isLoading = true;
                }
            });
        ...
        clearFilters() {
            ...
            this.sortBy = 'ItemTypeName';
            this.sortAsc = true;
            this.fetchList();
        },

        /**
         * Fetch item type list with current filters, sorting, and pagination
         * Uses HTMX to load partial view into container
         * @param {number} page - Page number to fetch (default: 1)
         * @returns {void}
         */
        fetchList(page = 1) {
            const container = document.getElementById('item-type-list-container');
            if (!container) return;
            ...
            const url = `/ItemTypes/List?${params.toString()}`;

            // Use htmx to fetch
            htmx.ajax('GET', url, {
                target: '#item-type-list-container',
                swap: 'innerHTML',
        ...
        /**
         * Export item types to Excel with current filters and sorting
        ...
        async exportToExcel() {
            ...
            try {
                ...
                const response = await fetch(`/api/ItemTypes/export?${params.toString()}`, {
                    method: 'GET',
                    headers: {
                ...
                a.href = url;
                a.download = `ItemTypes_${new Date().toISOString().slice(0, 10)}.xlsx`;
                ...
                console.error('[Item Type Index] Export error:', error);
...
window.itemTypeIndex = itemTypeIndex;
